...
`docs/viewer/index.html` in a clone. The viewer's CHM is drawn at 64 colour levels to keep the
page small; the GeoTIFF itself is not quantised.*
...
Python 3.12 or newer. Not on PyPI yet. The `uv sync` above is enough to run the sample and your
own LAZ, because `run` touches no network. Add `--extra dgt` for the catalogue commands, which pull
in the one network dependency, `requests`. Without a clone:
...
arguments, and the test suite exercises both forms.
...
the area, a coordinate that decodes to NaN or infinity, an area declaring no CRS of its own, a
`--cell` that does not divide the 1 m analysis cell, a missing `--attribution`. Every refusal and
its reason is in [`docs/inputs.md`](docs/inputs.md), with what has and has not been exercised.
DGT is the one provider this has been run against; any other is untested rather than unsupported.
The core's independence from this one site was measured rather than assumed, and the same page
says how.

## Results

The 0.5.0 run, over four raw DGT LAZ tiles at Sistelo, Arcos de Valdevez, Portugal: 845,372,695
bytes from a single sortie. DGT's own published rasters are not an input; nothing in this
repository reads them. The commands and their verbatim output are in
[`docs/live-smoke.md`](docs/live-smoke.md), and the machine-readable record is
[`docs/viewer/provenance.json`](docs/viewer/provenance.json). How the
tiles were obtained, and why acceptance was a byte-for-byte size check rather than a status code,
is in [`SITE.md`](SITE.md). `select` and `precheck` read the catalogue and need no account; only
the download does, and it is manual.
...
at a kappa of 0.991 over a six-tile test area. The official ground class is the reference the run
is quantified against and never an input to the filter; the noise classes are the exception, and
they are removed upstream of it. The filter is not the product either: it is re-derived from the
raw returns so that every published cell has a traceable origin, and the claim is the basis band
and the record rather than the surface. How it was chosen, what it replaced and how it scores
beside PDAL's own two are in [`docs/ground-filter.md`](docs/ground-filter.md).
...
- The ground filter does not remove every building, and the record calls what it keeps measured. At
  ...
  return publish as measured; the filter this tool ran until 0.5.0 published 87.7% of them, both
  re-derived by one instrument ([record](docs/reference-instrument-result.md)). No parameter of
  that filter fixed it, since the best single height threshold separates a roof from the terrain it
  stands on at 0.712 balanced accuracy ([diagnosis](docs/ground-filter-diagnosis.md)), so the
  repair was to change the filter. It surfaced only on a second area chosen for what
...
- Terrace preservation is measured, not enforced. The filter it replaced guaranteed survival by
  ...
  cannot tell a riser from a slope steep enough to span it: over an 800 m zone, 42.4% of the cells
  passing that test sit within 0.30 m of a plane ([result](docs/sharp-step-result.md)). That share
  is measured on a population without the measured-basis term the 95.082% carries, so it describes
  the range test and not this row's denominator; adding the planarity term to the 95.082%'s own
  population leaves it at 94.2%.
...
- The hash does not cover the attribution string either. Two runs differing only in
  ...
  anchor.
- The published grid overhangs the area you ask for, for two reasons that add up.
  `grid_for_bounds` floors the origin and ceils the count, which alone overhangs by up to one cell
  a side; snapping to whole 1 m blocks then adds up to `(1 m / --cell) - 1` more per axis.
  Membership is decided against the grid rather than the polygon, so a cell added either way
  publishes what was measured in it rather than undetermined.
...
- Interpolated cells borrow the nearest measured value, never farther than 2 m, with no smoothing.
  ...
  bands.
- The area is a single sortie, so `mixed_epochs` is false. An area mixing flight dates would be a
  mosaic of moments; the pipeline refuses it unless told to accept it, and the record declares it.
...
NASADEM all ship one, and Copernicus goes further than this by naming the donor dataset for every
filled pixel. What is missing is the tool layer, where GDAL, PDAL, GRASS, SAGA and WhiteboxTools
all fill voids without handing back a band saying which cells they invented. The quotes, the
mapping onto those conventions and the objections a domain reader will raise are in
