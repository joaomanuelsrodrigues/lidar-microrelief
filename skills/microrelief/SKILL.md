...
- `--cell` must divide the 1 m analysis cell, so it takes 1/k metres (1, 0.5, 0.25, 0.2, …) and
  **refuses** anything else, naming what would work. Ask for a cell size, do not round one. The
  grid then grows outward to whole 1 m blocks, **adding** up to `(1 m / --cell) - 1` cells per
  axis on top of the up-to-one-cell overhang the grid already had: one more at `0.5`, four at
  `0.2`, nine at `0.1`. Report that when the AOI's exact extent matters to the user.
