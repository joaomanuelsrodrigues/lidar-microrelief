...
# `known_limitations` also changes, because the read guards append two declared gaps to it. It is
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="require known_limitations to be the old list plus exactly the two gaps the "
        "0.4.0 guards declare. OFF by default, because a run-to-run control compares two builds of "
