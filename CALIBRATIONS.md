...
| `max_cells` | 200,000,000 | `grid.py` | **A cell count, not a memory bound — and the rationale was measured in 0.4.1 and found 7.5× too small.** The row read "one float64 array of that many cells is ~1.6 GB"; the run holds **ten** such arrays, not one: `grid.BYTES_PER_CELL` is **60 B/cell** measured (40 B across the accumulator's five float64/int64 arrays, 20 B across `CellStats`'s five float32/int32 ones, locked by `tests/test_accumulate.py`), so the ceiling is **~12 GB** before the ground filter, the two distance transforms and the surfaces. A grid under the ceiling can still be OOM-killed; that is declared in `known_limitations` and the README rather than defended against | Bound bytes rather than cells, or keep declaring it |
